    ...
    if (name === "courseId") {
      ...
      if (course) {
        ...
        setFormData(prev => ({
          ...
          amount: totalWithCharge,
          discountAmount: discountAmount,
          couponCode: "", // Reset coupon when course changes
        }));
      } else {
        setFormData(prev => ({
          ...prev,
  ...
  try {
    const response = await fetch(`${import.meta.env.VITE_API_URL}/coupons/apply`, {
      ...
      body: JSON.stringify({
        ...
        totalAmount: priceDetails.totalWithCharge, // Send only code and totalAmount
      }),
    });
    ...
    if (result.success) { // Note: backend returns { success: result.isValid, ...result }
      ...
      setFormData(prev => ({
        ...prev,
        amount: finalAmount,
        discountAmount: priceDetails.discountAmount + discountAmount,
      }));
      ...
      {selectedCourse && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Apply a coupon code for additional discount
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="flex flex-col md:flex-row gap-3">
              <div className="flex-1">
                <Input
                  ...
                  value={formData.couponCode}
                  onChange={(e) => {
                    // Auto-uppercase the coupon code
                    ...
                    // Reset coupon state if code changes
